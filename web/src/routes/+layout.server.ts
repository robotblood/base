...
export const load: LayoutServerLoad = async () => {
	const [stats, design, customTables, serverModules, templates] = await Promise.all([
		...
		// Markdown templates for the editor's "/" menu (Admin → Templates).
		api.getSetting('templates').catch(() => null)
	]);
